...
- [ ] Проверено, что `apps/bright_os_app/components.json` остаётся source of truth для default shadcn style, CSS variables, aliases, registries и Lucide icons.
- [ ] Проверены existing Bright OS components и `apps/bright_os_app/src/shared/ui`.
- [ ] Visible controls/surfaces используют registry-native source из Bright OS, shadcn/ui или Motion Primitives, а не hand-rolled `button`/`motion.button`/`div` + Tailwind.
...
- [ ] Hardcoded registry/demo colors (`black`, `white`, `zinc`, hex/rgb/arbitrary color utilities) заменены на semantic Bright OS tokens перед product use.
- [ ] Edits ограничены import/path compatibility, Bright OS data/actions и явно запрошенными content changes.
