...
Use one shared page workspace for authenticated product sections. It owns an opaque fixed header, a centered 1024px main column without a panel, and an equal 50/50 desktop split when a persistent or transient panel exists. Mobile panels reuse the shared sheet below the header.
