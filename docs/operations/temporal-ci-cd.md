...
DB/Supabase/Brai API variables. Verify the effective boundary without printing
environment values:
...
9. After generic deployment passes, the workflow records `goal_agents_deploy_started` and runs an independent Goal-agent restart/verification activity. It proves five exact environment pollers/builds, the API context poller, worker-deployment promotion, and one bounded non-destructive real agent-to-API cross-queue workflow on the deployed Temporal Server 1.29.x. A final activity may mark the slot `ready` only by compare-and-set on the same branch/SHA lease. The workflow then records `goal_agents_deploy_passed` and `preview_deploy_passed`; each phase reports only its own failure.
