...
const ACTIVITY_MD_PREVIEW_STORAGE_KEY = "bright_os_activity_md_preview";
...
export function loadActivityMarkdownPreviewMode(): boolean {
  ...
  try {
    return window.localStorage.getItem(ACTIVITY_MD_PREVIEW_STORAGE_KEY) === "true";
  } catch {
    return false;
  }
}
...
export function saveActivityMarkdownPreviewMode(enabled: boolean) {
  ...
  try {
    window.localStorage.setItem(ACTIVITY_MD_PREVIEW_STORAGE_KEY, enabled ? "true" : "false");
  } catch {
