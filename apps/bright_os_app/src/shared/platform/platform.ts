declare global {
  interface Window {
    ...
    BrightOsAndroidBack?: () => boolean;
    BrightOsAndroidTimerStop?: () => boolean;
  }
}
...
export function installAndroidBackHandler(handler: () => boolean): () => void {
  ...
  if (androidBackHandlers.length === 0) {
    previousAndroidBackHandler = window.BrightOsAndroidBack;
    window.BrightOsAndroidBack = runAndroidBackHandlers;
  }
  ...
  return () => {
    ...
    if (androidBackHandlers.length === 0 && window.BrightOsAndroidBack === runAndroidBackHandlers) {
      window.BrightOsAndroidBack = previousAndroidBackHandler;
