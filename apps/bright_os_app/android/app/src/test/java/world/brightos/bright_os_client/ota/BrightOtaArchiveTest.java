...
public class BrightOtaArchiveTest {
    @Test
    public void extractsSafeStaticBundle() throws Exception {
        File root = Files.createTempDirectory("bright-ota-safe").toFile();
        ...
        BrightOtaArchive.extractZip(archive, target, "index.html");
    ...
    public void rejectsPathTraversal() throws Exception {
        File root = Files.createTempDirectory("bright-ota-traversal").toFile();
        File archive = new File(root, "bundle.zip");
        ...
        BrightOtaException error = assertThrows(
            ...
            () -> BrightOtaArchive.extractZip(archive, new File(root, "bundle"), "index.html")
        );
    ...
    public void rejectsBackslashEntries() throws Exception {
        File root = Files.createTempDirectory("bright-ota-backslash").toFile();
        File archive = new File(root, "bundle.zip");
        writeZip(archive, new Entry("index.html", "ok"), new Entry("..\\escape.txt", "bad"));
    ...
    public void rejectsMissingEntrypoint() throws Exception {
        File root = Files.createTempDirectory("bright-ota-entrypoint").toFile();
        File archive = new File(root, "bundle.zip");
