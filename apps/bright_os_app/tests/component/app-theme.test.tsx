...
import { openSettingsFromProfile, setupBrightOsAppTest } from "./app-test-support";
...
describe("BrightOsApp theme", () => {
  ...
  it("ignores retired runtime theme token storage before the app hydrates", () => {
    ...
    window.localStorage.setItem("bright_os_theme_tokens", JSON.stringify({ primary: "#0f766e" }));
    window.localStorage.setItem("bright_os_accent_theme", JSON.stringify({ primary: "#7c3aed" }));
    new Function(scriptMatch?.[1] ?? "")();
  ...
  it("toggles the settings theme button through the app theme state", async () => {
    ...
    await waitFor(() => {
      expect(document.documentElement.dataset.theme).toBe("dark");
      expect(window.localStorage.getItem("bright_os_theme_mode")).toBe("dark");
    });
    ...
    await waitFor(() => {
      ...
      expect(window.localStorage.getItem("bright_os_theme_mode")).toBe("light");
    });
  });
