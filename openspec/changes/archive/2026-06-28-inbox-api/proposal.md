...
- `inbox` stores text as `explanation_text`, optional text/JSON content as `description_text`, stores whitelisted image/file attachments on disk, records attachment paths in `attachment_links`, stores source metadata and record type, and uses the raw text as the initial title.
